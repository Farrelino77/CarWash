...
const History = ({ navigation }) => {
  ...
  useEffect(() => {
    ...
    const unsubscribeParams = navigation.addListener('state', (e) => {
      const params = navigation.getState()?.routes?.find(r => r.name === 'Riwayat')?.params;
      if (params?.refresh) {
        fetchOrders();
        // Set active tab ke Diproses jika ada parameter activeTab
        if (params.activeTab) {
          setActiveTab(params.activeTab);
        }
        // Reset params setelah refresh
        navigation.setParams({ refresh: false, activeTab: undefined });
      }
    });
  ...
  const filterOrdersByStatus = () => {
    switch (activeTab) {
      ...
      case 'Diproses':
        return orders.filter(order => order.status === 'pending' || order.status === 'progress');
      case 'Cancel':
